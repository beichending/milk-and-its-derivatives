#!/usr/bin/env python3
"""Generate a self-contained SGX Anhydrous Milk Fat Futures business dashboard."""
...
def delivery_month_from_symbol(symbol: str) -> str | None:
    if len(symbol) < 6 or not symbol.startswith("AMF"):
        return None
...
def build_payload(
    ...
    if not history:
        raise RuntimeError("No SGX Anhydrous Milk Fat history available")
    ...
    return {
        "meta": {
            "title": "SGX-NZX Global Anhydrous Milk Fat Futures",
            ...
            "spread_contract_months": SPREAD_CONTRACT_MONTHS,
            "history_days": days,
            "source_url": "https://www.sgx.com/derivatives/products/dairy?cc=AMF",
...
<main class="shell">
  <header>
    <div>
      <div class="eyebrow">Daily Market Monitor · AMF</div>
      <h1>SGX Anhydrous Milk Fat Futures Dashboard</h1>
  ...
  <footer>
    <span>Source: SGX public derivatives data · Contract code AMF</span>
...
def main() -> int:
    ...
    parser.add_argument("--database", default="data/sgx_amf.sqlite3")
