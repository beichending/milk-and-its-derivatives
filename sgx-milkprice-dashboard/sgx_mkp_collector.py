#!/usr/bin/env python3
"""Daily SGX-NZX NZ Milk Price Futures collector and anomaly detector."""
...
from sgx_mkp_dashboard import generate_dashboard


API_BASE = "https://api.sgx.com/derivatives/v1.0"
PRODUCT_URL = "https://www.sgx.com/derivatives/products/dairy?cc=MKP"
    ...
    if lag > stale_limit:
        alerts.append(
            make_alert(
                ...
                "critical",
                "stale_market_data",
                f"SGX MKP data is {lag} business days old",
        ...
        if len(contracts) < reference_count * (1 - drop_ratio):
            alerts.append(
                make_alert(
                    ...
                    "contract_count_drop",
                    (
                        f"SGX returned {len(contracts)} MKP contracts; "
        ...
        for field in sorted(previous_fields - current_fields):
            alerts.append(
                make_alert(
                    ...
                    f"SGX MKP response no longer contains field: {field}",
        ...
        for field in sorted(current_fields - previous_fields):
            alerts.append(
                make_alert(
                    ...
                    "info",
                    "schema_field_added",
                    f"SGX MKP response contains a new field: {field}",
                    previous_business_date=previous_snapshot_date,
                )
            )
...
def export_snapshot(contracts: list[dict[str, Any]], business_date: str, output_dir: Path) -> Path:
    ...
    path = output_dir / f"sgx_mkp_{business_date}.csv"
...
def alert_text(alerts: list[dict[str, Any]]) -> str:
    lines = [f"SGX NZ Milk Price Futures alerts: {len(alerts)}"]
    lines.extend(
...
def send_notifications(alerts: list[dict[str, Any]], config: dict[str, Any]) -> None:
    ...
    if settings.get("smtp_host") and recipients:
        ...
        message["Subject"] = f"SGX NZ Milk Price Futures alert ({len(actionable)})"
...
def collect(config_path: Path, bootstrap: bool = True, notify: bool = True) -> int:
    ...
    try:
        ...
        if not contracts:
            raise RuntimeError("SGX returned no MKP contracts")
        ...
        raw_path = output_dir / f"sgx_mkp_{business_date}.json"
