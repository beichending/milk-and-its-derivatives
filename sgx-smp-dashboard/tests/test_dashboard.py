...
SPEC = importlib.util.spec_from_file_location(
    "dashboard", ROOT / "sgx_smp_dashboard.py"
)
...
class DashboardTests(unittest.TestCase):
    def setUp(self):
        ...
        for index, (symbol, delivery_month) in enumerate(self.symbols, start=1):
            self.connection.execute(
                ...
                (
                    ...
                    5000 + index,
                    5000 + index,
                    4995 if symbol == "SMPN26" else None,
                    5005 if symbol == "SMPN26" else None,
                    index,
                    100,
                ),
            )
        ...
        self.connection.execute(
            ...
            INSERT INTO history
            (business_date, symbol, settlement, volume, open_interest)
            VALUES ('2026-05-20', 'SMPM26', 4990, 2, 90)
            """
        )
    ...
    def test_payload_uses_nearest_twelve_contracts(self):
        ...
        self.assertEqual(payload["contracts"][0]["symbol"], "SMPN26")
        self.assertEqual(payload["contracts"][-1]["symbol"], "SMPM27")
        ...
        self.assertEqual(
            payload["views"]["2026-06-19"]["summary"]["front_symbol"], "SMPN26"
        )
    ...
    def test_historical_date_rolls_front_and_distant_contracts(self):
        self.connection.execute(
            ...
            INSERT INTO history
            (business_date, symbol, settlement, volume, open_interest)
            VALUES ('2025-11-20', 'SMPZ25', 4790, 1, 60)
            """
        )
        for index, symbol in enumerate(
            ["SMPF26", "SMPG26", "SMPH26", "SMPJ26", "SMPK26", "SMPM26", "SMPN26", "SMPQ26", "SMPU26", "SMPV26", "SMPX26", "SMPZ26"],
            start=1,
        ):
            self.connection.execute(
                """
        ...
        self.assertEqual(payload["summary"]["front_symbol"], "SMPF26")
        self.assertEqual(payload["summary"]["distant_symbol"], "SMPN26")
        self.assertEqual(len(payload["contracts"]), 12)
        self.assertEqual(payload["contracts"][-1]["symbol"], "SMPZ26")
    ...
    def test_generates_self_contained_html(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("SGX Skim Milk Powder Futures Dashboard", content)
            self.assertIn('"front_symbol":"SMPN26"', content)
            self.assertIn('"distant_symbol":"SMPF27"', content)
    ...
    def test_non_positive_price_is_treated_as_missing(self):
        self.connection.execute(
            ...
            INSERT INTO history
            (business_date, symbol, settlement, volume, open_interest)
            VALUES ('2026-06-18', 'SMPN26', 0, 8, 100)
            """
        )
        self.connection.commit()
        payload = dashboard.build_payload(
