# SGX Skim Milk Powder Futures daily collector

This tool collects every field returned for every active SGX-NZX Global Skim Milk Powder
Futures (`SMP`) contract, stores daily history in SQLite, exports JSON/CSV, and
runs historical anomaly checks.

## Data source

- Product page: <https://www.sgx.com/derivatives/products/dairy?cc=SMP>
- Current contracts: `https://api.sgx.com/derivatives/v1.0/contract-code/SMP?category=futures`
...
& "C:\Users\dbcmi\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" `
  .\sgx_smp_collector.py --config .\config.json status
```
...
- `data/sgx_smp_YYYY-MM-DD.json`: unmodified API response
- `data/sgx_smp_YYYY-MM-DD.csv`: all fields for all active contracts
- `data/sgx_smp.sqlite3`: snapshots, history, all fields, alerts, run audit
...
bootstrap also downloads expired monthly SMP contracts so retrospective dates
