#!/usr/bin/env python3
"""Generate a self-contained SGX Skim Milk Powder Futures business dashboard."""
...
def delivery_month_from_symbol(symbol: str) -> str | None:
    if len(symbol) < 6 or not symbol.startswith("SMP"):
        return None
...
def build_payload(
    ...
    if not history:
        raise RuntimeError("No SGX Skim Milk Powder history available")
    ...
    return {
        "meta": {
            "title": "SGX-NZX Global Skim Milk Powder Futures",
            ...
            "spread_contract_months": SPREAD_CONTRACT_MONTHS,
            "history_days": days,
            "source_url": "https://www.sgx.com/derivatives/products/dairy?cc=SMP",
...
<head>
  ...
  <title>SGX Skim Milk Powder Futures Monitor</title>
  <style>
    :root {
...
<main class="shell">
  <header>
    <div>
      <div class="eyebrow">Daily Market Monitor · SMP</div>
      <h1>SGX Skim Milk Powder Futures Dashboard</h1>
  ...
  <footer>
    <span>Source: SGX public derivatives data · Contract code SMP</span>
...
def main() -> int:
    ...
    parser.add_argument("--database", default="data/sgx_smp.sqlite3")
